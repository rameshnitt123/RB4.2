define([], function() {
  ...
  NewUser_PresentationController.prototype.createNewUser = function(userName, password, reenter) {
    var validationUtility = applicationManager.getValidationUtilManager();
    var newUserBM = applicationManager.getNewUserBusinessManager();
    var controller = applicationManager.getPresentationUtility().getController('frmOBsetUserNamePwd', true);
    ...
    if (isValidUserName && isValidPassword && scope_NewUserPresentationController.userAvailable) {
      ...
      newUserBM.createCredentialsForNewUser(scope_NewUserPresentationController.userData, scope_NewUserPresentationController.createNewUserPresentationSuccessCallback, scope_NewUserPresentationController.createNewUserPresentationErrorCallback);
    }
  };
  NewUser_PresentationController.prototype.createNewUserPresentationSuccessCallback = function(res) {
    scope_NewUserPresentationController.NUOLogin(scope_NewUserPresentationController.userNamePasswordJson);
  };
  ...
  NewUser_PresentationController.prototype.createUserSelectProducts = function(userproducts,productslist) {
    ...
    for (var i = 0; i < userproducts.length; i++) {
      var index = productslist.findIndex(function(item) {
        return item.productId === userproducts[i];
      });
    ...
    var navMan = applicationManager.getNavigationManager();
    navMan.setCustomInfo("selectedUserProduct", {productsCount: list.length});
  ...
  NewUser_PresentationController.prototype.validateAndSetPersonalInfo = function(data) {
    ...
    if (isDOBValid && isAgeValid) {
      var dob = date.split("/");
      data.dateOfBirth = dob[2] + "-" + dob[0] + "-" + dob[1];
      scope_NewUserPresentationController.updateNewUserModel(data);
      scope_NewUserPresentationController.commonFunctionForNavigation("frmOBResidentialAddress");
    } else {
      var controller = applicationManager.getPresentationUtility().getController('frmOBManuallyPersonalInfo', true);
  ...
  NewUser_PresentationController.prototype.NUOLogin=function(newUserJSON)
  {
    applicationManager.getPresentationUtility().showLoadingScreen();
  ...
  NewUser_PresentationController.prototype.navOnClickManuallySelected = function(){
    ...
    scope_NewUserPresentationController.commonFunctionForNavigation("frmOBManuallyPersonalInfo");
  };
