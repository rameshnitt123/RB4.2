define(["AsyncManager/BusinessControllers/BusinessController"], function(AsyncManager) {
  ...
  Transfer_PresentationController.prototype.checkExistingAccountwithAccountNumberSuccess = function(res){
    var accList = res;
    var navMan = applicationManager.getNavigationManager();
    var accdata =  navMan.getCustomInfo("frmTransfersToAccount");
    var accountNumber = accdata.selectedAccountData.accountNumber;
  ...
  Transfer_PresentationController.prototype.checkExistingAccountwithAccountNumberFailure = function(err){
    applicationManager.getPresentationUtility().dismissLoadingScreen();
  ...
  Transfer_PresentationController.prototype.checkExistingAccountwithIBAN = function(IBAN){
    if (IBAN.length >= 0) {
      applicationManager.getPresentationUtility().showLoadingScreen();
      var data={};
      data.searchKeyword = IBAN;
      var searchString = {
        searchString: data.searchKeyword
      }
      var criteria = kony.mvc.Expression.eq("searchString", data.searchKeyword) ;
      applicationManager.getAccountManager().fetchExternalAccountsByCriteria(criteria, scope_TransfersPresentationController.checkExistingAccountwithIBANSuccess, scope_TransfersPresentationController.checkExistingAccountwithIBANFailure);
    }
  };
  Transfer_PresentationController.prototype.checkExistingAccountwithIBANSuccess = function(res){
    var accList = res;
    var navMan = applicationManager.getNavigationManager();
  ...
  Transfer_PresentationController.prototype.checkExistingAccountwithIBANFailure = function(err){
    applicationManager.getPresentationUtility().dismissLoadingScreen();
    if (err["isServerUnreachable"]) {
      applicationManager.getPresentationInterruptHandler().showErrorMessage("postLogin", err);
    }
   ...
   Transfer_PresentationController.prototype.getSelectedFrequencyIndex = function() {
    ...
    Transfer_PresentationController.prototype.navigateToExternalFlow = function(type) {
        ...
        accMan.fetchExternalAccounts(function() {}, function(err) {
      	...
      	navMan.navigateTo("frmTransfersToAccountEurope");
        scope_TransfersPresentationController.externalAccount = false;
    };
  ...
  Transfer_PresentationController.prototype.switchDurationType = function(index) {
        ...
        switch (index) {
            ...
            case applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.frequency.NumberofRecurrence"):
                ...
                navMan.navigateTo("frmTransfersRecurrenceEurope");
                break;
        }
    };
...
//                 var formatedDate = forUtility.getFormatedDateString(dateobj, forUtility.getApplicationDateFormat());
            ...
            case applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.frequency.Daily"):
                ...
                transactionObj.setTransactionAttribute("isScheduled","1");
                navMan.navigateTo("frmTransfersDurationEurope");
                break;
   ...
   Transfer_PresentationController.prototype.navAfterToAcc = function() {
        ...
        if ((preAccData === "") || (preAccData === undefined) || (preAccData === null) || ((accountList.type === applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.TransfersEurope.ToMyDBXAccount")) && (accountList.selectedAccountData["accountID"] === preAccData["accountID"]))) {
            applicationManager.getPresentationUtility().showLoadingScreen();
   ...
   Transfer_PresentationController.prototype.setNickName=function(nickName){
    ...
    Transfer_PresentationController.prototype.cancelCommon = function() {
        ...
        var navManager = applicationManager.getNavigationManager();
      	navManager.setCustomInfo("frmTransfersToAccount",{});
	...
	Transfer_PresentationController.prototype.makeATransfer = function(description,feePaidByrecipient) {
        var transactionManager = applicationManager.getTransactionManager();
        transactionManager.setTransactionAttribute("transactionsNotes",description);
  		if(feePaidByrecipient){
          var configManager = applicationManager.getConfigurationManager();
          transactionManager.setTransactionAttribute("fee",configManager.internationalTransactionFee);
  ...
  Transfer_PresentationController.prototype.fetchBankDetailsForDomesticTransferFailure = function (err) {
    ...
    Transfer_PresentationController.prototype.isValidSwiftCode = function(swiftCode, formName) {
	 ...
	 Transfer_PresentationController.prototype.isValidIBAN = function(IBAN, formName) {
        ...
        if (validationUtility.isValidIBAN(IBAN)) {
            ...
            var controller = applicationManager.getPresentationUtility().getController(formName, true);
            controller.bindGenericError(applicationManager.getPresentationUtility().getStringFromi18n("kony.mb.TransfersEurope.InvalidIBAN"));
            return false;
        }
    };  
    ...
    Transfer_PresentationController.prototype.evaluateMinMaxAmountLimits = function(amount,selectedCurrency){
      var configManager =  applicationManager.getConfigurationManager();
      var baseCurrency = configManager.getBaseCurrency();
