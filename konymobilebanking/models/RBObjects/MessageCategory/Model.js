define([],function(){
	...
	function MessageCategory(defaultValues){
		var privateState = {};
	...
	BaseModel.isParentOf(MessageCategory);
	
	//Create new class level validator object
	BaseModel.Validator.call(MessageCategory);
	
	var registerValidatorBackup = MessageCategory.registerValidator;
	
	MessageCategory.registerValidator = function(){
		var propName = arguments[0];
		if(!setterFunctions[propName].changed){
			var setterBackup = setterFunctions[propName];
			setterFunctions[arguments[0]] = function(){
				if( MessageCategory.isValid(this, propName, val) ){
	...
	MessageCategory.relations = relations;
	
	MessageCategory.prototype.isValid = function(){
		return MessageCategory.isValid(this);
	};
	
	MessageCategory.prototype.objModelName = "MessageCategory";
