define([],function(){
	...
	function MemberShip(defaultValues){
		var privateState = {};
	...
	BaseModel.isParentOf(MemberShip);
	
	//Create new class level validator object
	BaseModel.Validator.call(MemberShip);
	
	var registerValidatorBackup = MemberShip.registerValidator;
	
	MemberShip.registerValidator = function(){
		var propName = arguments[0];
		if(!setterFunctions[propName].changed){
			var setterBackup = setterFunctions[propName];
			setterFunctions[arguments[0]] = function(){
				if( MemberShip.isValid(this, propName, val) ){
	...
	MemberShip.relations = relations;
	
	MemberShip.prototype.isValid = function(){
		return MemberShip.isValid(this);
	};
	
	MemberShip.prototype.objModelName = "MemberShip";
