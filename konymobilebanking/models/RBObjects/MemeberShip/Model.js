define([],function(){
	...
	function MemeberShip(defaultValues){
		var privateState = {};
	...
	BaseModel.isParentOf(MemeberShip);
	
	//Create new class level validator object
	BaseModel.Validator.call(MemeberShip);
	
	var registerValidatorBackup = MemeberShip.registerValidator;
	
	MemeberShip.registerValidator = function(){
		var propName = arguments[0];
		if(!setterFunctions[propName].changed){
			var setterBackup = setterFunctions[propName];
			setterFunctions[arguments[0]] = function(){
				if( MemeberShip.isValid(this, propName, val) ){
	...
	MemeberShip.relations = relations;
	
	MemeberShip.prototype.isValid = function(){
		return MemeberShip.isValid(this);
	};
	
	MemeberShip.prototype.objModelName = "MemeberShip";
