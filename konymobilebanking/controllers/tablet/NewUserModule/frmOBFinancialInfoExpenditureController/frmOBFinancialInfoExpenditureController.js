define({ 
  ...
  setMasterDataToSegment: function() {
    ...
    var segData = montlyExpenditure.map(function(item){
      return {
    ...
    if (self.selectedExpenditure !== -1) {
      segData[self.selectedExpenditure].flxMain.skin = "sknFlxF4F4F4Radius100";
    }
    self.view.segDependents.data = segData;
  ...
  processAndNavigate: function() {
    var self = this;
    var data = {
      "montlyExpenditure" : self.view.segDependents.selectedItems[0].selectedValue,
    ...
    segData.forEach(function(item) {
      ...
      item.flxMain.skin = type === data.montlyExpenditure ? "sknFlxF4F4F4Radius100" : "slFbox";
    });
  ...
  assignDataToForm: function(newUserJSON) {
    var montlyExpenditure = newUserJSON.montlyExpenditure ? newUserJSON.montlyExpenditure : "";
    var segData = this.view.segDependents.data;
    segData.forEach(function(item) {
      var type = item.selectedValue;
      item.flxMain.skin = type === montlyExpenditure ? "sknFlxF4F4F4Radius100" : "slFbox";
    });    
