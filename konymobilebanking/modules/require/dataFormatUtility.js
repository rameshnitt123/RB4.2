define(function () {
  ...
  var RedBlackTree = function(comparator) {
    ...
    RedBlackTree.prototype.preOrder = function() {
...
*/
    ...
    RedBlackTree.prototype.getAllKeys = function() {
      var arr = [];
    ...
    RedBlackTree.prototype.get = function(k) {
      if(k === null) {
        return null;
      }
  ...
  var makeGroups = function(keys, dataArray, comparator) {
    var tree = new RedBlackTree(comparator);
    var i;
    for(i in dataArray) {
  ...
  var makeGroupsWithMultipleKeys = function(keysArr, dataArray, comparator) {
    if(comparator === undefined || comparator === null || typeof(comparator) !== "function") {
    ...
    for(i in dataArray) {
      ...
      for(var j in keysArr) {
        valuesArr.push(dataArray[i][keysArr[j]]);
      }
      tree.insert(valuesArr, dataArray[i]);
    }
    var allkeys = tree.getAllKeys();
    var groups = [];
    for(i in allkeys) {
  ...
  return {
    ...
    "makeGroupsWithMultipleKeys": makeGroupsWithMultipleKeys
  };
